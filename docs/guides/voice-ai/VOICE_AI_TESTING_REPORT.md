...
WHISPER_PORT=9001
ASR_MODEL=small  # Whisper Small (244M params, автопунктуация)
...
2. **Whisper small model (244M)** - лучший выбор для production:
   - ✅ Автоматическая пунктуация (запятые, тире, точки)
   - ✅ Высокая точность 90-95% (vs 70-80% на tiny)
   - ✅ Быстро на CPU: 2-4s для коротких команд
   - ✅ Улучшает качество парсинга LLM благодаря правильной пунктуации
