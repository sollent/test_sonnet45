...
class LLMService
{
    ...
    private const SYSTEM_PROMPT = <<<'PROMPT'
        ...
        7. create_subtask - добавить подзадачу к УЖЕ СУЩЕСТВУЮЩЕЙ задаче в системе
           ИСПОЛЬЗУЙ ТОЛЬКО КОГДА: родительская задача УЖЕ СОЗДАНА ранее в системе
           НЕ ИСПОЛЬЗУЙ КОГДА: пользователь говорит "Создай задачу X и добавь ей подзадачу Y" - это create_task с subtasks!
           КРИТИЧЕСКИ ВАЖНО: "Создай задачу ... и добавь подзадачу" = create_task с параметром subtasks!
           Параметры:
           - parent_search (поиск родительской задачи - ОБЯЗАТЕЛЬНО!)
           - title (название подзадачи)

        8. create_multiple_subtasks - создать несколько подзадач для УЖЕ СУЩЕСТВУЮЩЕЙ задачи
           ⚠️ КРИТИЧЕСКИ ВАЖНО:
           - "СОЗДАЙ задачу X и добавь ей подзадачи Y, Z" = create_task с параметром subtasks!
           - "Добавь подзадачи Y, Z к задаче X" = create_multiple_subtasks (только если X уже существует!)
           - Если пользователь начинает с "СОЗДАЙ/СОЗДАТЬ" - используй create_task!
        ...
        "Создай задачу сходить на митинг и добавь ей две подзадачи: купить одежду и примериться в гардеробе" →
        {"action":"create_task","parameters":{"title":"Сходить на митинг","subtasks":[{"title":"Купить одежду"},{"title":"Примериться в гардеробе"}]},"confidence":0.93}
